...
@HiltViewModel
class NBChannelWiseViewModel @Inject constructor(
    ...
    private val _response = _request.switchMap { request ->
        repository.getChannelWiseNB(request)

    }
    val response: LiveData<Resource<ChannelwiseNBRes>> = _response
    fun getChannelWiseNB(request: CommonReq) {
