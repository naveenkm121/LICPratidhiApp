...
@HiltViewModel
class AnandaViewModel @Inject constructor(
    ...
    private val _response = _request.switchMap { request ->
        repository.getAnandaData(request)

    }
    val response: LiveData<Resource<AnandaRes>> = _response
    fun getAnandaData(request: CommonReq) {
