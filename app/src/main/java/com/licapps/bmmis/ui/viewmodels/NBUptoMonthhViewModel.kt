...
@HiltViewModel
class NBUptoMonthhViewModel @Inject constructor(
    ...
    private val _response = _request.switchMap { request ->
        ...
        when (_whichrequest) {
            0 -> repository.getNBperformanceMonthByG(request)
            1 -> repository.getNBperformanceMonthByV(request)
            2 -> repository.getNBperformanceMonthByA2B(request)
